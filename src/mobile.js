...
document.addEventListener('DOMContentLoaded',()=>{
 ...
 function collapsible(container,heading,children,key,collapsed=false){
  ...
  const render=()=>{const shut=mobile.matches&&collapsed;body.hidden=shut;button.setAttribute('aria-expanded',String(!shut));button.setAttribute('aria-label',(shut?'Expandir ':'Contraer ')+heading.textContent.trim());button.innerHTML='<i aria-hidden="true" class="fa-solid '+(shut?'fa-xmark':'fa-bars')+'"></i>';};
 ...
 function position(){if(mobile.matches){footer.append(top,version);$('settings-tab').append(rfid,recovery);}else{topMarker.after(top,version);marker.after(rfid,recovery);}}
